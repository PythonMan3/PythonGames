    ...
    def readMap(self, filename):
        with open(filename) as fi:
            ...
            for row in range(self.nrow):
                line = fi.readline()
                self.mapDataTop.append([int(tok) for tok in line.split(",")])
    def saveMap(self):
...
def draw_selection(messageEngine, screen, palette, cursor, fieldMap, mapSaved):
    # draw mapchip
...
def main():
    ...
    while True:
        ...
        if state == STATE_MAP:
            ...
            fieldMap.update()
            fieldMap.draw()
            draw_selection(messageEngine, screen, palette, cursor, fieldMap, fieldMap.mapSaved)
