    ...
    def battle_cmd_handler(self, event):
        ...
        if event.type == KEYDOWN and event.key == K_SPACE:
            ...
            if self.battle.cmdwnd.command == BattleCommandWindow.ATTACK:  # たたかう
                self.msgwnd.set("まだたたかえません/せんとうをじっそうしてない。")
...
class Battle:
    ...
    def __init__(self, msgwnd, msg_engine):
        ...
        status = ["ゆうしゃ", 16, 0, 1]
        # 戦闘ステータスウィンドウ
        self.status_wnd = BattleStatusWindow(Rect(90, 8, 104+32, 136), status, self.msg_engine)
    ...
    def start(self):
        ...
        self.msgwnd.set("とんでスライムが　あらわれた。")
        self.play_bgm()
    def update(self):
        pass
    def draw(self, screen):
...
class BattleStatusWindow(Window):
    ...
    def draw(self, screen):
        ...
        if self.is_visible == False: return
        # ステータスを描画
        status_str = [self.status[0], "H  %3d" % self.status[1], "M  %3d" % self.status[2], "LV %3d" % self.status[3]]
        for i in range(0, 4):
            dx = self.text_rect[0]
            dy = self.text_rect[1] + (self.LINE_HEIGHT+MessageEngine.FONT_HEIGHT) * (i % 4)
